...
let LogService = class LogService {
    ...
    async getLogs(authUser, projectId) {
        this.checkAuth(authUser);
        const roleName = authUser.role?.name;
        if (!roleName || !['Employee', 'Manager', 'Owner'].includes(roleName)) {
            throw new common_1.UnauthorizedException('Only Employee, Manager, and Owner roles can access this feature');
        }
        if (projectId === null ||
            projectId === undefined ||
    ...
    async getRecentLogsForOwner(authUser, projectId) {
        this.checkAuth(authUser);
        const roleName = authUser.role?.name;
        if (!roleName || roleName !== 'Owner') {
            throw new common_1.UnauthorizedException('Only Owner role can access this feature');
        }
        if (projectId === null ||
            projectId === undefined ||
            Number.isNaN(Number(projectId))) {
