...
async createTask(CreateTaskDto: CreateTaskDto, authUser: AuthenticatedUser) {
  ...
  if (!user.company?.id) {
    throw new BadRequestException('Assigned employee must be associated with a company');
  }
  ...
  if (!project.company?.id) {
    throw new BadRequestException('Project must be associated with a company');
  }
...
async updateTaskById(updateTaskDto: UpdateTaskDto, authUser: AuthenticatedUser, id: number) {
  this.checkAdmin(authUser);

  const task = await this.taskRepo.findOne({
    where: { id },
    relations: ['project', 'project.company', 'project.company.owner', 'assignedTo'],
  });
  ...
  if (roleName === 'Owner') {
    ...
    if (!task.project?.company?.owner?.id) {
      throw new ForbiddenException(
        'Task is not associated with any company',
      );
    }
  ...
  if (updateTaskDto.assignedToUserId) {
    ...
    if (roleName === 'Owner') {
      ...
      if (!newAssignedUser.company?.owner?.id) {
        throw new ForbiddenException(
          'Assigned user\'s company has no owner'
        );
      }
